    ...
    <header className="fixed top-0 left-0 right-0 z-50">
      <div className="bg-[#0A0A0A]/80 backdrop-blur-md border-b border-[#2C2C2C] px-6 md:px-10">
        <div className="flex justify-between items-center py-4 max-w-7xl mx-auto">
          <div>
            <p className='header font-bold  text-[20px] text-[#F8F8F8]'>Validly</p>
          </div>

          {/* Desktop Nav */}
          <ul className='hidden lg:flex gap-8 sub-text font-normal text-[12px] text-[#9A9A9A] uppercase '>
          ...
          <div className='hidden lg:flex sub-text font-normal text-[14px] gap-6 items-center uppercase '>
      ...
      {isOpen && (
        <div className="fixed inset-0 top-[65px] bg-[#0A0A0A] z-40 lg:hidden animate-in fade-in slide-in-from-top-4 duration-300">
          <nav className="flex flex-col items-center justify-center h-full gap-8 p-6">
            <ul className='flex flex-col items-center gap-8 sub-text font-medium text-[16px] text-[#9A9A9A] uppercase '>
            ...
            <div className='flex flex-col items-center gap-6 w-full max-w-[280px]'>
              <Link href="/login" onClick={() => setIsOpen(false)} className='text-[#9A9A9A] uppercase  text-[14px]'>Login</Link>
